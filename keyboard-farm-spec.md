# 键盘农场 (Keyboard Farm) — 开发规范 v3.0

> 本文档是给 Claude Code 的完整开发指令。
> 参考对象：Steam《指尖农场 / Typing Farmer》
> 技术栈：Tauri 2.x (Rust) + te s 3 + TypeScript + Vite

---

## 一、产品概述

**定位**：macOS / Windows 桌面后台放置游戏。用户启动后，游戏以**不规则透明窗口**常驻桌面，形如一座浮空农场岛。用户在任何应用中打字或点击鼠标时，游戏全局监听输入，将其转化为农场作物的生长动力。

**核心体验**：只要你在工作，农场就在耕作。农场岛直接"贴"在桌面上，周围透明，可以看到底下的应用窗口。

**视觉风格**：手绘卡通风，粗线条，暖色调。完整键盘被包裹在一个"农场岛"造型里——泥土边框、木栅栏围绕，键盘上方有建筑装饰群（谷仓、钟楼、大树等），动物在键帽间行走。整个岛底部有草地土壤截面。支持日夜变化。

**美术资产**：通过 Doubao-Seedream-5.0-lite AI 生成，统一手绘卡通风格。

**竞品参考**：`competitor/` 目录存放了指尖农场的 6 张游戏截图、宣传素材和竞品分析文档，开发中随时查阅以对齐视觉风格和交互设计。

### 1.1 UI/交互设计要点（对齐指尖农场）

> 对照 `competitor/screenshots/ss0-main-view.jpg` ~ `ss5-night-mode.jpg`

1. **不规则透明窗口**：Tauri 窗口 `transparent: true` + `decorations: false`，Phaser canvas 透明背景。只在农场岛区域有可见内容，周围透明可穿透点击。
2. **农场岛造型**：整个可见区域是一个农场岛——泥土+木边框包裹键盘，上方建筑群，两侧装饰（左侧栅栏+猪，右侧水井+猫），底部木栅栏+草地截面。不是矩形窗口。
3. **窗口拖拽**：在建筑群区域 / 岛边框上按住拖拽移动窗口位置（Tauri `startDragging()`）。
4. **窗口缩放**：支持通过滚轮或快捷键缩放整个农场岛大小。
5. **按键计数气泡**：每个键帽上方有小气泡显示该键按了几次（如 ss2-growth.jpg）。
6. **功能按钮融入岛**：统计、设置等按钮做成木牌/标签融入建筑群（右上角），不用独立 HUD 栏。
7. **面板弹出**：商店/任务/统计等面板从侧面弹出，木质边框风格，不遮挡整个窗口。
8. **动物行走**：解锁的动物在键帽区域随机游走，与键盘融为一体。
